...
describe("CalendarMonth", () => {
  ...
  it("shows up to 3 chips + overflow on a busy day", () => {
    ...
    render(CalendarMonth, { props: { month, today, meetings } });
    expect(screen.getByText("Анна Л.")).toBeInTheDocument();
    expect(screen.getByText(/\+1 ещё/)).toBeInTheDocument(); // 4 → 3 chips + "+1 ещё"
  });

  it("clicking a chip calls onSelect with the meeting id", async () => {
    const onSelect = vi.fn();
    render(CalendarMonth, { props: { month, today, meetings: [mtg("m1", "2026-06-10T09:00:00Z")], onSelect } });
    await fireEvent.click(screen.getByText("Анна Л."));
    expect(onSelect).toHaveBeenCalledWith("m1");
  });

  it("clicking «+N ещё» calls onOpenDay with a Date for that day", async () => {
    const onOpenDay = vi.fn();
    const day = "2026-06-10T09:00:00Z";
    const meetings = [mtg("a", day, "Анна Лебедева"), mtg("b", day, "Борис Петров"), mtg("c", day, "Вера Сидорова"), mtg("d", day, "Глеб Орлов")];
    render(CalendarMonth, { props: { month, today, meetings, onOpenDay } });
    await fireEvent.click(screen.getByText(/\+1 ещё/));
...
describe("CalendarList", () => {
  it("renders rows and fires onSelect", async () => {
    const onSelect = vi.fn();
    render(CalendarList, { props: { meetings: [mtg("m1", "2026-06-10T09:00:00Z")], onSelect } });
  ...
  it("shows an empty state", () => {
    render(CalendarList, { props: { meetings: [] } });
