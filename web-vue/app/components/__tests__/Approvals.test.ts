...
import ScoresReadonly from "../approvals/ScoresReadonly.vue";
import ApprovalDetail from "../approvals/ApprovalDetail.vue";
import RejectDialog from "../approvals/RejectDialog.vue";
...
describe("ScoresReadonly", () => {
  it("renders rows with mismatch pills; no note when some self exists", () => {
    render(ScoresReadonly, { props: { scores: SCORES as never } });
  ...
  it("shows the no-self note only when no block has a self-assessment", () => {
    ...
    render(ScoresReadonly, { props: { scores: allNull as never } });
...
describe("ApprovalDetail", () => {
  it("renders member header, decision, summary and actions", () => {
    render(ApprovalDetail, { props: { item: PENDING as never, busy: false } });
  ...
  it("fires onApprove from the confirm dialog with effects listed", async () => {
    const { emitted } = render(ApprovalDetail, { props: { item: PENDING as never, busy: false } });
    await fireEvent.click(screen.getByRole("button", { name: "Согласовать" }));
    ...
    expect(emitted().approve).toEqual([["r1"]]);
  });
});

describe("RejectDialog", () => {
  it("disables submit until a comment is entered", async () => {
    const { emitted } = render(RejectDialog, { props: { busy: false } });
    const btn = screen.getByRole("button", { name: "Вернуть лиду" });
    expect(btn).toBeDisabled();
    await fireEvent.update(screen.getByLabelText("Причина возврата"), "Мало свидетельств");
    expect(btn).toBeEnabled();
    await fireEvent.click(btn);
    expect(emitted().submit).toEqual([["Мало свидетельств"]]);
  });
});
