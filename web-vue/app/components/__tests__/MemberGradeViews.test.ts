...
import BlockProfile from "../grades/BlockProfile.vue";
import CompaBand from "../grades/CompaBand.vue";
import GradeEmptyState from "../grades/GradeEmptyState.vue";

describe("Member grade views", () => {
  it("BlockProfile marks above-grade, target and mastered segments", () => {
    render(BlockProfile, {
      props: { gradeOrd: 4, targetOrd: 5, levelCount: 7, blocks: [{ name: "Базы данных", cur: 5 }] },
    });
  ...
  it("BlockProfile shows a target gap when below target", () => {
    render(BlockProfile, {
      props: { gradeOrd: 2, targetOrd: 3, levelCount: 7, blocks: [{ name: "Архитектура", cur: 2 }] },
    });
    expect(document.querySelectorAll('[data-seg="target"]').length).toBe(1);
  });

  it("CompaBand positions the marker by compa", () => {
    render(CompaBand, { props: { compa: 0.62, gradeCode: "IC5" } });
