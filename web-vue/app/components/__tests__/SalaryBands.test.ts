...
import BandsEditor from "../grades/BandsEditor.vue";
...
describe("GradeBands", () => {
  it("masked: bars + spread, no ₸ numbers", () => {
    render(GradeBands, { props: { levels: MASKED, taxRate: null } });
  ...
  it("exact: shows gross and net (net = gross × (1 − rate))", () => {
    render(GradeBands, { props: { levels: EXACT, taxRate: 0.1 } });
...
describe("BandsEditor", () => {
  it("edits a band field and fires the setter", async () => {
    const { emitted } = render(BandsEditor, { props: { levels: BANDS, taxPct: 10 } });
    await fireEvent.update(screen.getByLabelText("IC1 медиана"), "400000");
    expect(emitted().band).toEqual([[1, { band_mid: 400000 }]]);
  });

  it("edits the tax rate", async () => {
    const { emitted } = render(BandsEditor, { props: { levels: BANDS, taxPct: 10 } });
    await fireEvent.update(screen.getByLabelText("Ставка ИПН, %"), "12");
    expect(emitted().tax).toEqual([[12]]);
  });

  it("shows a live net figure (gross × (1 − rate))", () => {
    render(BandsEditor, { props: { levels: BANDS, taxPct: 10 } });
...
describe("GradesClient bands gating", () => {
  it("shows «Редактировать вилки» only on the Вилки tab when canEditBands", async () => {
    render(GradesClient, { props: { canEdit: false, canEditBands: true } });
    expect(screen.queryByRole("button", { name: "Редактировать вилки" })).not.toBeInTheDocument();
    await fireEvent.click(screen.getByRole("button", { name: "Вилки" }));
    expect(screen.getByRole("button", { name: "Редактировать вилки" })).toBeInTheDocument();
  });

  it("hides «Редактировать вилки» without canEditBands", async () => {
    render(GradesClient, { props: { canEdit: false, canEditBands: false } });
    await fireEvent.click(screen.getByRole("button", { name: "Вилки" }));
    expect(screen.queryByRole("button", { name: "Редактировать вилки" })).not.toBeInTheDocument();
  });

  it("bands-edit hides the framework «Редактировать» entry (mutual exclusivity)", async () => {
    render(GradesClient, { props: { canEdit: true, canEditBands: true } });
    await fireEvent.click(screen.getByRole("button", { name: "Вилки" }));
    // both entry points available on the bands tab
    expect(screen.getByRole("button", { name: "Редактировать вилки" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Редактировать" })).toBeInTheDocument();
    // enter bands-edit → framework entry disappears, badge shows
    await fireEvent.click(screen.getByRole("button", { name: "Редактировать вилки" }));
