...
import ReviewHistory from "../grades/ReviewHistory.vue";
import GradeHero from "../grades/GradeHero.vue";
...
describe("ReviewHistory", () => {
  it("renders rows with decisions and the pending pill", () => {
    render(ReviewHistory, { props: { reviews: [PENDING, FINAL] as never, codeOf } });
  ...
  it("renders the empty state", () => {
    render(ReviewHistory, { props: { reviews: [], codeOf } });
    expect(screen.getByText(/Ревью ещё не проводились/)).toBeInTheDocument();
  });

  it("skips drafts", () => {
    render(ReviewHistory, { props: { reviews: [{ ...FINAL, status: "draft" }] as never, codeOf } });
    expect(screen.getByText(/Ревью ещё не проводились/)).toBeInTheDocument();
  });

  it("final rows show the resolved date", () => {
    render(ReviewHistory, { props: { reviews: [FINAL] as never, codeOf } });
...
describe("GradeHero review action", () => {
  it("offers to open a review when none is active", () => {
    render(GradeHero, { props: { ...HERO, activeReview: null, onOpenReview: () => {} } });
    expect(screen.getByRole("button", { name: "Открыть ревью" })).toBeInTheDocument();
  });
  it("offers to continue a draft", () => {
    render(GradeHero, { props: { ...HERO, activeReview: "draft", onOpenReview: () => {} } });
  ...
  it("shows the pending pill instead of a button", () => {
    render(GradeHero, { props: { ...HERO, activeReview: "pending", onOpenReview: () => {} } });
  ...
  it("shows the returned pill for a draft returned by HR", () => {
    render(GradeHero, { props: { ...HERO, activeReview: "draft", returned: true, onOpenReview: () => {} } });
